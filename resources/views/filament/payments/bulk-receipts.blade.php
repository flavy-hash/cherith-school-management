...
<head>
    ...
    <title>Bulk Payment Receipts - Cherith Junior School</title>
    <style>
        ...
        .print-btn { display: block; width: 220px; margin: 20px auto; padding: 10px 20px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; }
            ...
            <div class="receipt-container">
                ...
                <div class="tra-info">
                    ...
                    @if(is_array($receipt) && ! empty($receipt) && (($receipt['error'] ?? false) !== true))
                        ...
                        @endif
                    @else
                        <p><strong>Receipt Number:</strong> {{ (is_array($receipt) ? ($receipt['receiptNumber'] ?? $receipt['receipt_number'] ?? null) : null) ?? ($payment->transaction_id ?? 'PAY-' . $payment->id) }}</p>
                        <p><strong>Verification Code:</strong> {{ is_array($receipt) ? ($receipt['verificationCode'] ?? $receipt['verification_code'] ?? 'N/A') : 'N/A' }}</p>
                        <p><strong>Status:</strong> Unable to fetch SmartEFD receipt.</p>
                        @if(is_array($receipt))
                            <p><strong>Code:</strong> {{ $receipt['code'] ?? 'N/A' }}</p>
                            @if(isset($receipt['message']) && $receipt['message'])
